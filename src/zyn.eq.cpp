...
ZynAudioEffectX::ZynAudioEffectX(audioMasterCallback audioMaster)
    : AudioEffectX(audioMaster, EQ_NUM_PRESETS, 1) // 1 program, 1 parameter only
{
    setNumInputs(2);       // stereo in
    setNumOutputs(2);      // stereo out
    setUniqueID('ZYEQ');   // identify
    ...
    _effectMgr->changeeffect(EffectTypes::EQType);
}
...
{
    strcpy_s(name, kVstMaxEffectNameLen, "Zyn.EQ");
    return true;
}
...
{
    strcpy_s(text, kVstMaxProductStrLen, "Zyn.EQ");
    return true;
}
